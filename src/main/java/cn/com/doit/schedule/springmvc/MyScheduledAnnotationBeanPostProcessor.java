...
//用以实现spring自带schedule动态配置
public class MyScheduledAnnotationBeanPostProcessor implements
		BeanPostProcessor, Ordered, EmbeddedValueResolverAware,
	...
	private final MyScheduledTaskRegistrar registrar = new MyScheduledTaskRegistrar();
	...
	private void finishRegistration() {
		...
		if (this.registrar.hasTasks() && this.registrar.getScheduler() == null) {
			Assert.state(this.beanFactory != null,
					"BeanFactory must be set to find scheduler by type");
			try {
				// Search for TaskScheduler bean...
				this.registrar.setTaskScheduler(this.beanFactory
						.getBean(TaskScheduler.class));
			} catch (NoUniqueBeanDefinitionException ex) {
				try {
					this.registrar.setTaskScheduler(this.beanFactory.getBean(
				...
				try {
					this.registrar.setScheduler(this.beanFactory
							.getBean(ScheduledExecutorService.class));
				} catch (NoUniqueBeanDefinitionException ex2) {
					throw new IllegalStateException(
							"More than one ScheduledExecutorService bean exists within "
		...
		if (!this.nonAnnotatedClasses.contains(targetClass)) {
			...
			if (annotatedMethods.isEmpty()) {
				...
				if (logger.isTraceEnabled()) {
					logger.trace("No @Scheduled annotations found on bean class: "
							+ bean.getClass());
				}
			} else {
				// Non-empty set of methods
				for (Map.Entry<Method, Set<Scheduled>> entry : annotatedMethods
						.entrySet()) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(annotatedMethods.size()
	...
	protected void processScheduled(Scheduled scheduled, Method method,
			Object bean) {
		try {
			...
			if (StringUtils.hasText(initialDelayString)) {
				...
				if (this.embeddedValueResolver != null) {
					initialDelayString = this.embeddedValueResolver
							.resolveStringValue(initialDelayString);
				}
				try {
			...
			if (StringUtils.hasText(cron)) {
				...
				this.registrar.addCronTask(new CronTask(runnable,
			...
			if (fixedDelay >= 0) {
				...
				processedSchedule = true;
				this.registrar.addFixedDelayTask(new IntervalTask(runnable,
			...
			if (StringUtils.hasText(fixedDelayString)) {
				...
				processedSchedule = true;
				if (this.embeddedValueResolver != null) {
					fixedDelayString = this.embeddedValueResolver
							.resolveStringValue(fixedDelayString);
				}
				try {
			...
			if (fixedRate >= 0) {
				Assert.isTrue(!processedSchedule, errorMessage);
				processedSchedule = true;
				this.registrar.addFixedRateTask(new IntervalTask(runnable,
			...
			if (StringUtils.hasText(fixedRateString)) {
				...
				processedSchedule = true;
				if (this.embeddedValueResolver != null) {
					fixedRateString = this.embeddedValueResolver
							.resolveStringValue(fixedRateString);
				}
				try {
	...
	public static String getDefaultTaskSchedulerBeanName() {
		return DEFAULT_TASK_SCHEDULER_BEAN_NAME;
	}

	public MyScheduledTaskRegistrar getRegistrar() {
		return registrar;
	}

	public Set<Class<?>> getNonAnnotatedClasses() {
		return nonAnnotatedClasses;
	}
